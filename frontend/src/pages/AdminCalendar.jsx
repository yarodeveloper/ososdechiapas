...
const AdminCalendar = () => {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center italic" style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>Sincronizando Agenda...</div>;
        ...
        <div className="min-h-screen pb-20 overflow-x-hidden transition-colors duration-300" style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>
            <header className="fixed top-0 left-0 w-full backdrop-blur-xl border-b z-50 transition-colors" style={{ backgroundColor: 'var(--bg-header)', borderColor: 'var(--border-main)' }}>
                <div className="max-w-md mx-auto px-6 py-5 flex justify-between items-center">
                    <button onClick={() => navigate('/admin/dashboard')} className="w-10 h-10 rounded-xl flex items-center justify-center transition-colors" style={{ backgroundColor: 'var(--bg-card)', color: 'var(--text-dim)' }}>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {['match', 'training', 'travel'].map(t => (
                                    <button 
                                        ...
                                        className={`py-3 rounded-xl text-[8px] font-black uppercase tracking-widest transition-all border ${formData.event_type === t ? 'bg-red-600 text-white border-transparent' : 'text-zinc-600'}`}
                                        style={{ borderColor: formData.event_type === t ? '' : 'var(--border-main)' }}
                                    >
                                        {t.toUpperCase()}
                                    </button>
                                ))}
                            </div>
                            <input required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full border rounded-2xl py-4 px-5 text-sm outline-none focus:border-red-600 font-bold" placeholder="Título del evento" style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-main)' }} />
                            <div className="grid grid-cols-2 gap-4">
                                <select required value={formData.category_id} onChange={e => setFormData({...formData, category_id: e.target.value})} className="border rounded-2xl py-4 px-4 text-[10px] font-black uppercase outline-none focus:border-red-600" style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>
                                    <option value="">CATEGORÍA</option>
                                    {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                                <input required type="datetime-local" value={formData.start_time} onChange={e => setFormData({...formData, start_time: e.target.value})} className="border rounded-2xl py-4 px-4 text-[10px] outline-none focus:border-red-600 font-black" style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-dim)' }} />
                            </div>

                            {formData.event_type === 'match' && (
                                <select required value={formData.rival_id} onChange={e => setFormData({...formData, rival_id: e.target.value})} className="w-full border rounded-2xl py-4 px-5 text-[10px] font-black uppercase outline-none focus:border-red-600" style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>
                                    <option value="">SELECCIONAR RIVAL</option>
                                    {teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                </select>
                            )}

                            <input required value={formData.location_name} onChange={e => setFormData({...formData, location_name: e.target.value})} className="w-full border rounded-2xl py-4 px-5 text-sm outline-none focus:border-red-600 font-bold" placeholder="Lugar (Ej: Estadio Central)" style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-main)' }} />
                            
                            <button type="submit" className="w-full bg-red-600 py-5 rounded-[2rem] font-black uppercase text-xs tracking-widest active:scale-95 transition-all shadow-xl shadow-red-900/20 text-white">
