...
const Payments = () => {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center italic" style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>Sincronizando Finanzas...</div>;

    return (
        <div className="min-h-screen font-body pb-32 selection:bg-red-600 overflow-x-hidden transition-colors duration-300" style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>
            <header className="fixed top-0 left-0 w-full backdrop-blur-xl border-b z-50 transition-colors" style={{ backgroundColor: 'var(--bg-header)', borderColor: 'var(--border-main)' }}>
                <div className="max-w-md mx-auto px-6 py-5 flex justify-between items-center">
                    <button onClick={() => navigate('/admin/dashboard')} className="transition-colors" style={{ color: 'var(--text-dim)' }}>
            ...
            <main className="max-w-md mx-auto px-6 pt-24 space-y-8">
                    ...
                    <section className="card p-8 animate-slide-up shadow-2xl border-2" style={{ borderColor: 'var(--border-main)' }}>
                        ...
                        <form onSubmit={handleNewCharge} className="space-y-6">
                            ...
                            <input 
                                ...
                                className="w-full border rounded-2xl py-4 px-5 text-sm font-bold outline-none focus:border-red-600" 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <input 
                                    ...
                                    className="w-full border rounded-2xl py-4 px-5 text-sm font-black outline-none focus:border-red-600" 
                                    placeholder="Monto $" 
                                    style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-main)' }}
                                />
                                <input 
                ...
                {pendingValidations.length > 0 && (
                    <section className="p-6 rounded-[2rem] space-y-4 animate-fade shadow-2xl border" style={{ backgroundColor: 'rgba(245, 158, 11, 0.1)', borderColor: 'rgba(245, 158, 11, 0.3)' }}>
                ...
                <section className="space-y-4 pb-20">
                    {filteredPayments.map(p => (
                        <div key={p.id} className={`card border rounded-[2.5rem] p-6 flex flex-col gap-6 shadow-2xl transition-all border-2`} style={{ borderColor: p.status === 'validating' ? 'rgba(245, 158, 11, 0.4)' : 'var(--border-main)' }}>
                            <div className="flex justify-between items-start">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <h3 className="text-base font-black uppercase italic tracking-tight" style={{ color: 'var(--text-main)' }}>{p.parent_name}</h3>
                                    <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-dim)' }}>{p.description}</p>
                                </div>
                            ...
                            <div className="pt-5 border-t flex flex-col gap-4" style={{ borderColor: 'var(--border-main)' }}>
                                {p.receipt_url && (
                                ...
                                <div className="flex justify-between items-center px-1">
                                    ...
                                    <div className="flex gap-3">
                                        {p.status !== 'paid' && !p.receipt_url && (
                                            <button onClick={() => updateStatus(p.id, 'paid')} className="hover:text-green-500 flex items-center gap-1 text-[9px] font-black uppercase tracking-widest transition-all" style={{ color: 'var(--text-dim)' }}>
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>
                                                Liquidar
                                            </button>
                                        )}
                                        <button onClick={() => deletePayment(p.id)} className="transition-colors" style={{ color: 'var(--text-muted)' }}>
            ...
            <nav className="fixed bottom-0 left-0 w-full backdrop-blur-xl border-t px-6 pt-4 pb-8 z-50 rounded-t-[2.5rem] transition-colors" style={{ backgroundColor: 'var(--nav-bg)', borderColor: 'var(--border-main)' }}>
                <div className="max-w-md mx-auto flex justify-between items-center px-4">
                    <Link to="/admin/dashboard" className="flex flex-col items-center gap-1.5" style={{ color: 'var(--text-dim)' }}>
                    ...
                    <Link to="/admin/settings" className="flex flex-col items-center gap-1.5" style={{ color: 'var(--text-dim)' }}>
