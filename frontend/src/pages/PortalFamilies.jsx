...
const PortalFamilies = () => {
    ...
    return (
        <div className="min-h-screen font-outfit pb-32 overflow-x-hidden uppercase" style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>
            <header className="px-6 py-10 flex justify-between items-center border-b backdrop-blur-xl sticky top-0 z-50 transition-colors" style={{ backgroundColor: 'var(--nav-bg)', borderColor: 'var(--border-main)' }}>
            ...
            <main className="max-w-md mx-auto px-6 pt-10 space-y-12 relative z-10">
                ...
                <section className="rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-main)' }}>
                    <div className="flex items-center gap-6 relative z-10">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] font-bold uppercase tracking-widest mt-2 italic" style={{ color: 'var(--text-dim)' }}>{user?.email}</p>
                        </div>
                    </div>
                </section>
                ...
                <section className="space-y-6">
                    <div className="flex justify-between items-center px-2">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.3em] italic" style={{ color: 'var(--text-dim)' }}>Mis Jugadores</h3>
                        <div className="w-1/2 h-px" style={{ backgroundColor: 'var(--border-main)' }}></div>
                    </div>

                    <div className="space-y-6">
                        {players.map(p => (
                            <div key={p.id} onClick={() => navigate(`/portal/player/${p.id}/playcard`)} className="rounded-[2.5rem] p-7 shadow-2xl relative group active:scale-95 transition-all cursor-pointer border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-main)' }}>
            ...
            <nav className="fixed bottom-0 left-0 w-full backdrop-blur-xl border-t px-6 pt-4 pb-10 z-50 rounded-t-[2.5rem] transition-colors" style={{ backgroundColor: 'var(--nav-bg)', borderColor: 'var(--border-main)' }}>
                <div className="max-w-md mx-auto flex justify-between items-center px-4 leading-none">
                    <button onClick={() => navigate('/portal')} className="flex flex-col items-center gap-2" style={{ color: 'var(--text-dim)' }}>
