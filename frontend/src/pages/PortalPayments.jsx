        ...
        <div className="bg-[#050505] text-white min-h-screen font-outfit pb-32 overflow-x-hidden">
            <header className="fixed top-0 left-0 w-full bg-black/80 backdrop-blur-xl border-b border-white/5 z-50">
                <div className="max-w-md mx-auto px-6 h-18 flex items-center justify-between py-5">
                    ...
                    <h1 className="text-xs font-black uppercase tracking-[0.3em] italic text-zinc-400">Estado de <span className="text-red-600">Cuenta</span></h1>
            ...
            <main className="max-w-md mx-auto px-6 pt-24 space-y-8">
                ...
                {/* ── Tabs ── */}
                <div className="flex bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 shadow-xl">
                    <button 
                        onClick={() => setView('pending')}
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${view === 'pending' ? 'bg-red-600 text-white shadow-lg shadow-red-900/40' : 'text-zinc-600'}`}
                    >
                        Deudas
                    </button>
                    <button 
                        onClick={() => setView('paid')}
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${view === 'paid' ? 'bg-red-600 text-white shadow-lg shadow-red-900/40' : 'text-zinc-600'}`}
                    >
                        Mis Recibos
                    </button>
                </div>
                    ...
                    <section className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <div className="space-y-4 pt-2">
                                <div><p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-1">Banco</p><p className="text-sm font-black uppercase">{bankInfo.bank_name}</p></div>
                                <div><p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-1">CLABE</p><p className="text-lg font-mono font-black text-white tracking-widest whitespace-nowrap">{bankInfo.bank_clabe}</p></div>
                                <div><p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-1">Titular</p><p className="text-xs font-bold text-zinc-300 uppercase italic line-clamp-1">{bankInfo.bank_holder}</p></div>
                            </div>
                        </div>
                    </section>
                )}
                ...
                <section className="space-y-4">
                    <div className="flex justify-between items-center px-2">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 italic">
                            {view === 'paid' ? 'Historial de Pagos' : 'Cargos Pendientes'}
                        </h3>
                        {view === 'pending' && <span className="text-2xl font-black italic tracking-tighter">$ {totalPending.toLocaleString()}</span>}
                        {view === 'paid' && <span className="text-2xl font-black italic tracking-tighter text-green-500">$ {totalPaid.toLocaleString()}</span>}
                    </div>

                    {filteredList.length === 0 ? (
                        <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-16 text-center italic opacity-30 text-xs uppercase font-black tracking-widest">
                            No hay {view === 'paid' ? 'recibos' : 'deudas'} registrados
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {filteredList.map(p => (
                                <div key={p.id} className={`bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-6 shadow-xl transition-all ${p.status === 'validating' ? 'border-amber-600/30' : ''}`}>
                                    <div className="flex justify-between items-start mb-6">
                                        <div>
                                            <h3 className="text-[15px] font-black tracking-tight uppercase italic text-white line-clamp-1">{p.description}</h3>
                                        ...
                                        <div className="text-right">
                                            <p className="text-xl font-black italic tracking-tighter text-white leading-none">$ {parseFloat(p.amount).toLocaleString()}</p>
                                    ...
                                    {p.status !== 'paid' && p.status !== 'validating' && (
                                        <Link 
                                            ...
                                            className="w-full bg-red-600/10 hover:bg-red-600/20 text-red-500 py-4 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all"
                                        >
                                    ...
                                    {p.status === 'paid' && p.receipt_url && (
                                        <a 
                                            ...
                                            className="w-full bg-zinc-900 py-4 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 active:scale-95 transition-all"
                                        >
