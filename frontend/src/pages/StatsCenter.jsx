        ...
        <div className="bg-black text-white min-h-screen pb-20 selection:bg-red-600 selection:text-white">
            ...
            <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-zinc-900 px-6 py-4">
                <div className="max-w-md mx-auto flex items-center justify-between">
                    <button onClick={() => navigate('/')} className="w-10 h-10 flex items-center justify-center bg-zinc-900 rounded-full active:scale-95 transition-transform">
            ...
            <main className="max-w-md mx-auto px-6 pt-8 animate-fade pb-32">
                ...
                <section className="mb-12">
                   <div className="flex items-center justify-between mb-4">
                      <h3 className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">Spotlight: MVP's {leaderboard.filter(p => p.mvp_count > 0).length === 0 && 'Globales'}</h3>
                      ...
                      {(leaderboard.filter(p => p.mvp_count > 0).length > 0 ? leaderboard.filter(p => p.mvp_count > 0) : globalMvps).slice(0, 5).map(p => (
                         <div key={p.id} onClick={() => navigate(`/players/${p.id}`)} className="min-w-[120px] flex flex-col items-center gap-3 cursor-pointer active:scale-95 transition-transform">
                            ...
                            <div className="text-center">
                                ...
                                {leaderboard.filter(p => p.mvp_count > 0).length === 0 && <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">{p.category_name}</span>}
                            </div>
                         </div>
                      ))}
                      {(leaderboard.filter(p => p.mvp_count > 0).length === 0 && globalMvps.length === 0) && (
                        <div className="w-full py-8 bg-zinc-950 border border-zinc-900 border-dashed rounded-3xl flex flex-col items-center justify-center opacity-40">
                ...
                {/* League Selector */}
                <div className="mb-8">
                    <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest block mb-4">Seleccionar Liga</span>
                ...
                {selectedLeague && (
                    <div className="mb-10">
                        <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest block mb-4">Categoría</span>
                ...
                <div className="flex gap-2 mb-8 bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900">
                    {[
                ...
                <section>
                    <div className="flex justify-between items-end mb-6">
                        ...
                        <img src="/icons/award-svgrepo-com.svg" className="w-6 h-6 opacity-40 brightness-0 invert" alt="Award" />
                    </div>

                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="h-24 bg-zinc-900/50 animate-pulse rounded-3xl border border-zinc-800"></div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-4">
                            {leaderboard.map((player, index) => (
                                ...
                                >
                                    <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-[2rem] p-4 flex items-center gap-4 overflow-hidden">
                                        {/* Rank */}
                                        <div className="absolute top-0 right-0 p-4 opacity-10 font-display font-black text-5xl italic">
                                            #{index + 1}
                                        </div>
                                        ...
                                        <div className="relative w-16 h-16 rounded-2xl overflow-hidden bg-zinc-800 border border-zinc-700 flex-shrink-0">
                                            {player.photo_url ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center text-zinc-600 font-display text-xl">
                                            ...
                                            <div className="flex gap-3 mt-2 items-center">
                                                {sortBy === 'touchdowns' && (
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-xl leading-none font-display font-black text-white">{player.total_touchdowns || 0}</span>
                                                    </div>
                                                )}
                                                {sortBy === 'yards' && (
                                                    <div className="flex flex-col">
                                                        <span className="text-[8px] font-black text-red-500 uppercase tracking-widest">Yardas Tot.</span>
                                                        <span className="text-xl leading-none font-display font-black text-white">{player.total_yards || 0}</span>
                                                    </div>
                                                )}
                                                {sortBy === 'tackles' && (
                                                    <div className="flex flex-col">
                                                        <span className="text-[8px] font-black text-red-500 uppercase tracking-widest">Tackles</span>
                                                        <span className="text-xl leading-none font-display font-black text-white">{player.total_tackles || 0}</span>
                                                    </div>
                                                )}
                                                
                                                <div className="h-6 w-px bg-zinc-800 mt-1"></div>
                                                
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">{sortBy === 'touchdowns' ? 'Yardas' : 'TDs'}</span>
                                                    <span className="text-xs font-display font-black text-zinc-300">{sortBy === 'touchdowns' ? (player.total_yards || 0) : (player.total_touchdowns || 0)}</span>
                                                </div>
                                                {(sortBy === 'touchdowns' || sortBy === 'yards') && (
                                                    <>
                                                        <div className="h-4 w-px bg-zinc-800 mt-1"></div>
                                                        <div className="flex flex-col">
                                                            <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Tck</span>
                                                            <span className="text-xs font-display font-black text-zinc-300">{player.total_tackles || 0}</span>
                                                        </div>
                                                    </>
                                                )}
                                            </div>
                                        ...
                                        {player.mvp_count > 0 && (
                                            <div className="absolute top-2 left-2">
                                                <div className="bg-red-600 text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shadow-lg shadow-red-900/40 translate-x-[-10px] translate-y-[-10px] rotate-[-15deg]">
                                                    MVP
                                                </div>
                                            </div>
                                        )}
                        ...
                        <div className="text-center py-20 bg-zinc-900/20 border border-dashed border-zinc-800 rounded-3xl">
                            <p className="text-zinc-500 text-sm font-medium">No hay estadísticas registradas <br/>para esta categoría aún.</p>
                        </div>
                    )}
                </section>
            </main>
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
                <button 
                   onClick={() => navigate('/')}
                   className="bg-white text-black px-8 py-4 rounded-full font-display text-xs uppercase tracking-widest shadow-[0_15px_40px_rgba(255,255,255,0.2)] active:scale-90 transition-all flex items-center gap-3"
                >
                    <img src="/icons/home-svgrepo-com.svg" className="w-4 h-4" alt="Home" />
                    Volver al Inicio
                </button>
            </div>
        </div>
